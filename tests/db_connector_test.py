...
class TestDBConnector(ut.TestCase):
    ...
    def test_insert_and_fetch_user(self):
        ...
        row = self.db.fetch_user(username)
    ...
    def test_authenticate_user(self):
        ...
        pw = 'pw3'
        self.db.insert_user(username, pw)
        self.assertTrue(self.db.authenticate_user(username, pw))
        self.assertFalse(self.db.authenticate_user(username, 'wrong'))
    ...
    def test_insert_and_fetch_messages_with_join_and_filter(self):
        ...
        # Prepare two users
        self.db.insert_user('u1', 'a')
        self.db.insert_user('u2', 'b')
        # Insert messages
        self.db.insert_message('u1', 'hello')
        self.db.insert_message('u2', 'world')
        ...
        ids = [row[0] for row in msgs]
        self.assertEqual(ids, sorted(ids))
        # Test last_update filter: fetch only messages with id > last_id_of_first_list
        last_id = ids[-1]
        # Add one more message
        self.db.insert_message('u1', 'again')
        newer = self.db.fetch_messages(last_update=last_id)
        self.assertTrue(all(row[0] > last_id for row in newer))
        self.assertTrue(any(row[2] == 'again' for row in newer))
